...
use database::paginated_models::{serialise_db_key, PaginatedDbRequest};
...
pub async fn get_houses(
    ...
    let service_req = PaginatedDbRequest {
        value: "Adelaide".to_string(),
        limit: pagination.limit,
        last_evaluated_key: pagination.decode_last_evaluated_key().map_err(map_error_to_response)?,
    };
    let db_response = HouseItem::list_by_city_from_db(service_req, db).await.map_err(map_error_to_response)?;
    let response = PaginatedResponse{
        items: db_response.items.into_iter().map(HouseResponse::from).collect(),
